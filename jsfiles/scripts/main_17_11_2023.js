...
function disabledAllButtons() {
  $(
    "#clsMnuBtn, #clsReplay, #clsBackBtn, #ToolBtn, #clsHelpBtn, #clsExitBtn, #clsAudioOnBtn, #id_sliderContainer, #clsPlayPauseBtn"
  ).css({
    "pointer-events": "none",
  });
}
...
function fnShowBtn() {
  $("#clsPageNumber,#clsBackBtn,#clsNxtBtn,#clsReplay").css({
    display: "block",
  });
}
...
function fnCreateXMLObjArr() {
    ...
    $(this)
      ...
      .each(function () {
        ...
        $(this)
          .find("TOPIC")
          .each(function () {
            gModuleArr[moduleCounter].Lessons[lessonCounter].Topics[
              topicCounter
            ] = $(this);
            ...
            $(this)
              ...
              .each(function () {
                gModuleArr[moduleCounter].Lessons[lessonCounter].Topics[
                  topicCounter
                ].Pages[pageCounter] = $(this);

                // Total page number will not count the page which will have "F" or "N" for attribute "SHOWINMENU"

                if (
                  String($(this).attr("SHOWINMENU")).toUpperCase() != "F" &&
                  String($(this).attr("SHOWINMENU")).toUpperCase() != "N"
                ) {
                  totalPageCounter++;
                }
  ...
  isPageCompletionOnPageLoad = oXML
    .find("isPageCompletionOnPageLoad")
    .text()
    .toLowerCase();
}
...
function fnSliderOrProgressBar() /* This function is used to define Slider Behaviour (Slider or Progress Bar) */ {
  ...
  if (
    ...
    isProgressBarDisabled == "f" ||
    isProgressBarDisabled == false
  ) {
    isProgressBarDisabled = false;
  } else if (
    isProgressBarDisabled == "true" ||
    isProgressBarDisabled == "t" ||
    isProgressBarDisabled == true
  ) {
    isProgressBarDisabled = true;
  }
...
function fnAddPlayerEvents() {
  ...
  $("#clsNxtBtn").click(function (e) {
    if (isActive(this)) fnGetPage("NEXT");
  });
  ...
  $("#clsAudioOffBtn,#clsAudioOnBtn").click(function (e) {
    /*if (isActive(this))
        toggleVolumeSlider(); */
    if (isActive(this)) fnSoundOnOff();
    //if (isActive(this)) muteAudio();
  });

  function muteAudio() {
    console.log(audioObjRef.muted);
    if (!audioObjRef.muted) {
  ...
  $("#clsExitBtn").click(function (e) {
    closeHelpBtnFunc();
    if ($("#ToolBtn").hasClass("ClsToolbtnSelected")) fnTools();
    if (isActive(this)) {
      var arrIOSVer = String(ver).split(",");

      if (
        parseInt(arrIOSVer[0]) == 8 &&
        parseInt(arrIOSVer[1]) >= 0 &&
        parseInt(arrIOSVer[1]) <= 2
      ) {
        BVScorm_adlOnunload();
  ...
  $("#searchTextDiv").click(function (e) {
    if (isAndroid2) jQuery.event.remove(window, "resize", resizeFrame);
  });

  $("#searchTextDiv").keypress(function (e) {
    if (e.which == 13) {
      /*the enter key code*/ e.preventDefault();
  ...
  $("#clsGlosBtn").click(function (e) {
    closeHelpBtnFunc();
    if (isActive(this)) fnLoadGlossaryPopup();
  });

  $("#clsRefBtn").click(function (e) {
    closeHelpBtnFunc();
    if (isActive(this)) fnLoadReferencesPopup();
  });

  $("#clsVideoRefBtn").click(function (e) {
    closeHelpBtnFunc();
    if (isActive(this)) fnLoadVideoReferencesPopup();
  });
  ...
  $("#clsPlayPauseBtn, .clsPause, .clsPlay").on("click", function (e) {
    if (!$("#clsPlayPauseBtn").hasClass("clsPlayDisable")) {
      ...
      if (videoObjectId == null) videoPlaying = false;
...
function fnLoadMenu() {
  ...
  $("<div class='menuCover'><div class='MenuPopup'></div></div>").insertAfter(
    ".gridContainer"
  );
  $(".menuCover").data("state", 0);
...
function fnLoadExitPopup() {
  ...
  $("<div class='ExitPopup'></div><div class='bgPopup'></div>").insertAfter(
    ".clsPreloader"
  );
...
function fnLoadGlossaryPopupPage(aCurrent) {
  ...
  if (
    aCurrent.attr("glossaryTitle") != null ||
    aCurrent.attr("glossaryTitle") != undefined
  ) {
  ...
  //console.log(_getDesc);
  var __html =
    '<div class="popupglossasryCnt"><div id="helptopbar"><div id="id_glosTitleText" class="titleCase ' +
    _getCurId +
    '">' +
    _getHeading +
    '</div><div class="close_txt"><span id="clsText">Close</span></div><div id="id_glosCloseBtn" class="id_glosCloseBtn" title="Close"></div></div><div class="glossaryCntTxt" id=' +
    _getCurId +
    ">" +
    _getDesc +
    "</div></div>";
...
function fnLoadHelpPopup() {
  ...
  fnPauseAudio();
  $("<div class='HelpPopup'></div><div class='bgHelpPopup'></div>").insertAfter(
    ".gridContainer"
  );
  ...
  switch (lCase) {
        ...
        if (gStartAssessmet) {
          if (gCurrQuestionCnt >= gShowQuestionCnt) {
            ...
            for (
              var i = 1;
              i <
              gModuleArr[lCurrModNum].Lessons[lCurrLessNum].Topics[lCurrTopNum]
                .Pages.length;
              i++
            ) {
              if (
          ...
          if (
            ...
            lCurrLessNum <=
            gModuleArr[lCurrModNum].Lessons.length - 1
          ) {
            lCurrLessNum++;
            lCurrTopNum = 0;
            lCurrPgNum = 0;
    ...
    case "PREV":
      ...
      if (
        ...
        gModuleArr[lCurrModNum].Lessons[lCurrLessNum].Topics[lCurrTopNum].Pages[
          lCurrPgNum
        ].attr("PREVPAGE") != undefined
      ) {
        var lPageId =
          gModuleArr[lCurrModNum].Lessons[lCurrLessNum].Topics[
            lCurrTopNum
          ].Pages[lCurrPgNum].attr("PREVPAGE");
      } else {
        if (lCurrPgNum > 0) {
          lCurrPgNum--;
        } else {
...
function fnInitialize() {
  ...
  $("#CourseTitleFrame").html(
    '<span id="spnCourseTitleTxt"  UNSELECTABLE="on">' + courseTitle + "</span>"
  );
...
function fnLoadPage(lModNum, lLessNum, lTopNum, lPageNum) {
  ...
  gIsAssessment =
    gModuleArr[lModNum].Lessons[lLessNum].Topics[lTopNum].attr(
      "ISSKILLCHECK"
    ) == "Y"
      ? true
      : false;
  ...
  gIsPageSkillCheck =
    gModuleArr[lModNum].Lessons[lLessNum].Topics[lTopNum].Pages[lPageNum].attr(
      "ISSKILLCHECK"
    ) == "Y"
      ? true
      : false;
  ...
  isNextDisable =
    oXML.find("DISABLEDNEXT").text().toLowerCase() == "false" ? true : false;
  ...
  if (
    ...
    if (IsScrom) {
      ...
      if (currentPageType != "assessment" && window.isReviewMode == false) {
        ...
        BVScorm_setlocation(
          gModuleArr[lModNum].Lessons[lLessNum].Topics[lTopNum].Pages[
            lPageNum
          ].attr("ID")
        );
  ...
  var isCurrPgComp = fnCheckPageComplection(
    gModuleArr[lModNum].Lessons[lLessNum].Topics[lTopNum].Pages[lPageNum].attr(
      "ID"
    )
  );
  ...
  var currPgTitle = gModuleArr[lModNum].Lessons[lLessNum].Topics[lTopNum].Pages[
    lPageNum
  ]
    .find("PAGETITLE")
    .text();
  ...
  var lessonTitle = gModuleArr[lModNum].Lessons[lLessNum]
    .find("LESSONTITLE")
    .text();

  var topicTitle = gModuleArr[lModNum].Lessons[lLessNum].Topics[lTopNum]
    .find("TOPICTITLE")
    .text();
  ...
  if (
    ...
    $("#CourseTitleFrame").html(
      '<span id="spnCourseTitleTxt"  UNSELECTABLE="on">' +
        courseTitle +
        "</span>"
    );
    ...
    $("#pgTitleFrame").html(
      '<span id="spnTopicTitleTxt"  UNSELECTABLE="on">' +
        currPgTitle +
        "</span>"
    );
  } else {
  ...
  } else setAudioButtonState(bAudioDisable);
...
function update508ForPlayerCtrl(p_strFocus) {
  ...
  if (
    $("#clsNxtBtn").hasClass("clsGlowNxtBtn") ||
    $("#clsNxtBtn").hasClass("clsEnaNxtBtn")
  )
...
function volumeProgressState(showhideSlider) {
  if (true) $("#id_slider-vertical").addClass("d-none");
...
function getNextPageId() {
  ...
  if (gStartAssessmet) {
    ...
    if (
      ...
      lPageId =
        gModuleArr[lCurrModNum].Lessons[
          lCurrLessNum + 1
        ].Topics[0].Pages[0].attr("ID");
    } else if (lCurrModNum < gModuleArr.length - 1) {
      lPageId =
        gModuleArr[lCurrModNum + 1].Lessons[0].Topics[0].Pages[0].attr("ID");
    }
  }
...
function fnSetPageComp() {
  ...
  if (
    fCheckCompletion(
      gModuleArr[gCurrMod].Lessons[gCurrLess].Topics[gCurrTop],
      "Topic"
    )
  ) {
    var lTopicId =
      gModuleArr[gCurrMod].Lessons[gCurrLess].Topics[gCurrTop].attr("ID");
...
function fnSetCoreData() {
  ...
  for (var i = 0; i < gPageTrackingArr.length; i++) {
    _trackedData[i] =
      gPageTrackingArr[i].id + "$" + gPageTrackingArr[i].trackingArr;
  }
  ...
  var dataSend =
    lPagesArrDummy.join(",") +
    "#" +
    lTopicsArrDummy.join(",") +
    "#" +
    lLessonsArrDummy.join(",") +
    "#" +
    modData +
    "#" +
    _trackedData.join("|") +
    ...
    gAssessmentAttempt +
    "#" +
    assessmentNextBtn.join(",") +
    "#" +
    assessment_score.join(",") +
    "#" +
    correctQueCounter +
    "#" +
    assesmentScore +
...
function fGetNonCompletedTopicPages() {
  ...
  for (var i = 0; i < gCompTopArr.length; i++) {
    ...
    for (var k = 0; k < lTopicObj.Pages.length; k++) {
      lPagesArrForCompletedTopics[lPagesArrForCompletedTopics.length] =
        lTopicObj.Pages[k].attr("ID");
    }
  }
...
function fGetNonCompletedLessonTopics() {
  ...
  for (var i = 0; i < gCompLessArr.length; i++) {
    ...
    for (var k = 0; k < lLessonObj.Topics.length; k++) {
      lTopicsArrForCompletedLessons[lTopicsArrForCompletedLessons.length] =
        lLessonObj.Topics[k].attr("ID");
    }
  }
...
function fGetNonCompletedModuleLessons() {
  ...
  for (var i = 0; i < gCompModArr.length; i++) {
    ...
    for (var k = 0; k < lModuleObj.Lessons.length; k++) {
      lLessonsArrForCompletedModules[lLessonsArrForCompletedModules.length] =
        lModuleObj.Lessons[k].attr("ID");
    }
  }
...
function setVideoObject(videoId) {
  ...
  $("#" + videoObjectId).bind(
    ...
    function (e) {
      ...
      if (event == "FullscreenOff") resizeFrame();
    }
  );
}
...
function fnCheckForBookMark() {
  ...
  if (IsScrom) {
    ...
    if (lesson_location != "" && lesson_location != undefined) {
      ...
      if (
        window.confirm("Do you want to go to the last visited page?") != true
      ) {
        lesson_location = "";
...
function fCheckCompletion(lObj, lCase) {
  switch (lCase) {
    ...
    case "Topic":
      return (
        lObj.Pages.length <= fGetCompleteElementCount(lObj.Pages, gCompPageArr)
      );

    case "Lesson":
      return (
        lObj.Topics.length <= fGetCompleteElementCount(lObj.Topics, gCompTopArr)
      );

    case "Module":
      return (
        lObj.Lessons.length <=
        fGetCompleteElementCount(lObj.Lessons, gCompLessArr)
      );

    case "Course":
      return (
        gModuleArr.length <= fGetCompleteElementCount(gModuleArr, gCompModArr)
      );
  }
}
...
function handlePageComplete() {
  ...
  if (RemovePageAnimation == false || RemovePageAnimation == "false") {
    ...
    TweenMax.from($("#contentContainer"), 0, {
      "margin-top": "0%",
      onComplete: startPageAnimComplete,
    });
  } else if (nextBtnClik) {
    ...
    TweenMax.to($("#contentContainer"), 2, {
      "margin-top": "0%",
      onComplete: startPageAnimComplete,
    });
  } else if (backBtnClik) {
    ...
    TweenMax.from($("#contentContainer"), 2, {
      "margin-top": "100%",
      onComplete: startPageAnimComplete,
    });
  }
...
function fnSoundOnOff() {
  if ($(".audioCnt").length == 1) {
    //console.log("fnSoundOnOff inside");
    checkAudioMuteUnmute1();
  } else {
    //console.log("fnSoundOnOff outside");
  }
  ...
  if (!bAudioMute) {
    ...
    /* $("#id_transcriptBox").show();
         ...
         $("#transcriptBtn span").html("TRANSCRIPT OFF");*/
    //isTransEnable = true;
    console.log("fnSoundOnOff in first");
    audioObjRef.muted = true;
    $("#clsAudioOnBtn").css("display", "none");
    $("#clsAudioOffBtn").css("display", "inline");
    ...
    console.log(" fnSoundOnOff in second");
    audioObjRef.muted = false;
    $("#clsAudioOffBtn").css("display", "none");
    $("#clsAudioOnBtn").css("display", "inline");
    $("#clsAudioOffBtn span.tooltip").html("Audio");
...
function UpdateTheVideoTime(time) {
  ...
  if (time > Number(updateContentArray[currCue].time)) {
    if (
      updateContentArray[currCue].transcript != "" &&
      updateContentArray[currCue].transcript != undefined
    ) {
...
var isTouchDevice = /ipad|iPhone|android/i.test(
...
function checkIpadAudio() {
  ...
  if (isTouchDevice && !isIntroPage) {
    ...
    var $a = $(
      '<a href="#" id="fakeClick"  style="position: absolute; top:40%;left:40%;z-index: 11000;"><img src="media/images/play_btn.png" /></a>'
    );
      ...
      for (var k = 0; k < lArrTopics.length; k++) {
        ...
        for (var l = 0; l < lArrPages.length; l++) {
          if (lArrPages[l].attr("MARKCOURSECOMPLETION") == "T") {
            return fCheckCompletion(
              gModuleArr[i].Lessons[j].Topics[k].Pages[l],
              "Page"
            );
          }
        }
...
function set(ID, value) {
  with (parent) {
    ...
    if (
      course.trackedObjects[course.currentSco.currentPage.trackingid] != 1 ||
      (course.trackedObjects[course.currentSco.currentPage.trackingid] == 1 &&
        course.currentSco.currentPage.parentItem.onePage != null)
    ) {
      ...
      if (
        course.trackedObjects[course.currentSco.currentPage.trackingid] == 1 &&
        course.currentSco.currentPage.updateProgress == true
      ) {
    ...
    if (
      course.lockdown == MODE_LOCK &&
      course.trackedObjects[course.currentSco.currentPage.trackingid] == 1 &&
      course.currentSco.currentPage.parentItem.onePage != true
    ) {
      unblock();
    } else if (
      course.lockdown == MODE_LOCK &&
      course.trackedObjects[course.currentSco.currentPage.trackingid] == 1 &&
      course.currentSco.currentPage.parentItem.onePage == true &&
      course.trackedObjects[
        course.currentSco.currentPage.parentItem.trackingid
      ] == 1
    ) {
      unblock();
    }
...
function setTranscriptButtonState(transState) {
  if (
    transState.toUpperCase() == "TRUE" ||
    transcriptDisable.toLowerCase() == "t"
  ) {
    fnhideTranscript();
...
function fnHideShellBtn(instanceObj, lcase) {
  lcase
    ? instanceObj.css("visibility", "hidden")
    : instanceObj.css("visibility", "visible");
}
...
function fSendCouseToServerPage() {
  ...
  $.ajax({
    ...
    url:
      "http://elseviernetcme.istream4.prev.datacon.nl/ElsevierNET-cme/certificate/CompleteCourse.aspx?CourseID=" +
      CourseID +
      "&UniqueID=" +
      UniqueID,
