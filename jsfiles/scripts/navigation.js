            ...
            $(this)
              ...
              .each(function () {
                if ($(this).attr("ID") == lPageId) {
                  ...
                  if (
                    $(this).attr("SHOWINMENU").toUpperCase() == "F" ||
                    $(this).attr("SHOWINMENU").toUpperCase() == "N"
                  ) {
                    gCurrPageNum--; //to solve page counter issue for assessment and result page
                  } else gCurrPageNum = totalPageCounter + 1;
...
function getCurPageName() {
  ...
  return gModuleArr[gCurrMod].Lessons[gCurrLess].Topics[gCurrTop].Pages[
    gCurrPage
  ]
    .find("PAGETITLE")
    .text();
...
function fnJumpToPageByID(lPageId, lIsHideMenu) {
  console.log("NAVIGATION : 1" + lPageId + "-----" + lIsHideMenu);
  ...
  fnShowPreLoader();
  console.log("NAVIGATION lPageId:" + lPageId);
  ...
  if (isPageAvailable) {
    ...
    console.log("FN CALL NAVIGATION");
    fnLoadPage(gCurrMod, gCurrLess, gCurrTop, gCurrPage, true);
  } else {
    console.log("FN CANN NAVIGATION : elase");
  }
